...
public:
    void sendData(int valeurCapteur, float lux) {
        if (client.connect(SERVER_IP, SERVER_PORT)) {
            Serial.println("Connecté au serveur");

            String xmlData = "<?xml version='1.0' encoding='UTF-8'?>";
            xmlData += "<donnees>";
            xmlData += "<valeur_capteur>" + String(valeurCapteur) + "</valeur_capteur>";
            xmlData += "<lux>" + String(lux) + "</lux>";
            xmlData += "</donnees>";

            String request = "POST /api.php HTTP/1.1\r\n";
            request += "Host: " + String(SERVER_IP) + "\r\n";
            request += "Content-Type: application/xml\r\n";
            request += "Content-Length: " + String(xmlData.length()) + "\r\n\r\n";
            request += xmlData;
...
void loop() {
  ...
  int valeurCapteur = analogRead(A0); // Lire le capteur
    float lux = valeurCapteur * 0.1; // Exemple de conversion
    sender.sendData(valeurCapteur, lux);
    delay(1000); // Envoyer les données toutes les 5 secondes
}